...
// ── NCA types / modes (from hacpack main.c) ──────────────────────────────────
// Each maps directly to NCA header content_type and section layout.
// Yanu uses only: PROGRAM (--plaintext) and META (no plaintext).
const NCA_TYPE = Object.freeze({
    PROGRAM: 0x00,
    META: 0x01,
    CONTROL: 0x02,
    DATA: 0x03,
...
export function buildIvfcHashTree(romfsData) {
    ...
    for (let lvl = 0; lvl < numLevels; lvl++) {
        ...
        ivfcView.setBigUint64(base + 0x00, BigInt(cumulativeOffset), true);
        ivfcView.setBigUint64(base + 0x08, BigInt(reversedSizes[lvl]), true);
...
// ── packNca dispatcher (hacpack modes) ───────────────────────────────────────
...
export function packNca(type, opts, keys, log) {
    const _log = typeof log === 'function' ? log : () => {};
    switch (type) {
        case NCA_TYPE.PROGRAM:
            return packPlaintextProgramNca(
                opts.exefsData, opts.romfsData, opts.controlData,
                opts.titleId, keys, log
            );
        case NCA_TYPE.META:
            return packMetaNca(
                opts.cnmtData, opts.pfs0FileName,
                opts.titleId, keys, log
            );
        // ── Stubs (not needed for yanu update pipeline) ──────────────────────
        case NCA_TYPE.CONTROL:
        case NCA_TYPE.DATA:
        case NCA_TYPE.MANUAL:
        case NCA_TYPE.PUBLICDATA:
            // hacpack: RomFS-only NCA from directory via romfs_build()
            // yanu: copies from update container as-is
            _log('info', `  [STUB] ${NCA_TYPE[type]} (RomFS-only) — not needed for --update`);
            return null;
        default:
            _log('warn', `  Unknown nca_type 0x${type.toString(16)}`);
            return null;
    }
}
...
function buildFsHeader(buf, opts) {
    buf.fill(0);
    buf[0x00] = 0x02;
    buf[0x01] = 0x01;
    buf[0x02] = opts.partitionType;
    buf[0x03] = opts.fsType;
    buf[0x04] = opts.cryptType;

    const view = new DataView(buf.buffer, buf.byteOffset);
    view.setBigUint64(0x40, BigInt(opts.sectionStart), true);
    view.setBigUint64(0x48, BigInt(opts.sectionSize), true);
}
