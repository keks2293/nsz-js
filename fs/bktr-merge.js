...
export async function mergeRomFS(baseNcaData, updateNcaData, options = {}) {
    ...
    return {
        merged,                              // full virtual image (buffered mode only) — for verification scripts
        mergedData: streaming ? null : merged.subarray(dataLevelOffset, dataLevelOffset + dataLevelSize), // level-5 data only — the RomFS image to repack
