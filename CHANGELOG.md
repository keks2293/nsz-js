...
1. **Feature: NSP merge + split, XCI inputs** — new `fs/merge.js` and `fs/split.js`, CLI `--merge`/`--split`, browser Mode switcher (Convert/Merge/Split). Merge unions members of 2+ NSPs (base/update/DLC) into one NSP with dedup by filename — no keys needed (mirrors FinalRom). `--merge` also accepts `.xci` inputs (any mix, e.g. base `.xci` + update `.nsp` → `.nsp`): the secure partition's HFS0 is read header-only and its files (NCAs, `.tik`, `.cert`, `.cnmt.xml`) become union members with the same first-wins dedup; `XCIReader` accepts any reader with `read()`, falls back to the backup HEAD header at `0x1100`, exposes `getSecureFiles()`. Split parses CNMT out of each meta-NCA (XTS header decrypt + AES-CTR section decrypt + inner PFS0), groups NCAs by title, writes one `{titleId}_{base|update|dlc}_v{version}.nsp` per title, attaches the matching `.tik`/`.cert` via rights-id lookup (cert matched by ticket filename stem). Both work over any reader/writer (browser blob/FSA/SW, CLI fd). Bugs fixed along the way: ticket parse on pooled `fs` buffers (`Ticket.parse(data.buffer)` saw the whole 64 KiB `Buffer.allocUnsafe` pool → `.tik`/`.cert` silently dropped in CLI split; fixed with `.slice()`), and CNMT `ContentEntry.type` offset (read at byte 53, spec says byte 54/0x36 — broke split's meta/deltaFragment filtering). Verified end-to-end in browser (Playwright: merge → 5 members deduped, split → per-title NSP with meta/program/tik/cert) and CLI (split→merge round-trip byte-identical member set).

2. **Refactor: dedupe NCA decrypt + CNMT parse into shared helpers** — `fs/nca.js`, `fs/split.js`, `fs/cnmt-hashes.js`, `fs/adapter.js`, `fs/ticket.js`. Both `split.js` and `cnmt-hashes.js` had inline copies of "XTS-decrypt first 0xC00 bytes with `header_key` → `NCAHeader.parse`" and "AES-CTR decrypt section with `section.cryptoKey`/`cryptoCounter` at `section.offset` + find inner PFS0 (at `section.sectionStart || 0x20`), parse first file as CNMT". Extracted into `decryptNcaHeader(raw, keys)`, `decryptNcaSection(data, section)`, `parseCnmtFromDecryptedSection(fsData, section, {allowRawPfs0})` and `readCnmtFromMeta(reader, entry, header)` in `fs/nca.js` (with `isPfs0` helper; `decryptNcaSection` keeps split semantics — returns raw data when `cryptoType === 1` or no `cryptoKey`; `cnmt-hashes` keeps its `try/catch` + `No titleKeyDec` guard and passes `{allowRawPfs0:true}`). `copyRange` + `COPY_CHUNK` (8 MiB chunked copy, duplicated in split/merge) moved to `fs/adapter.js`. `collectTicketsByRightsId` stays a local helper in `fs/split.js` — its only consumer — mirroring FinalRom's `NspUnmerger._loadTicketEntries` (parser `SwitchTicket` separate) and nscb_rust (split matches by filename, no content parse). `fs/ticket.js` now holds only the pure `Ticket` parser, throwing `Error('Invalid ticket format')` on unknown `signatureType` instead of silently defaulting to `0x100` (mirrors Python nsz `IOError`; previously an unknown type would misparse offsets and silently drop the ticket — now logged as "Skipping malformed ticket"). No behavior change on valid NSPs. Verified: build OK, synthetic split / merge-split / merge-xci / cnmt-hashes smoke tests + all crypto/convert tests PASS, CLI split → output NSP with 4 members (meta/program/tik/cert).

3. **CLI: mode hints in help (nscb_rust model)** — `nsz-cli.js`. `printUsage()` stays flat (all modes + options always listed), but mode-specific options are tagged in their description like nscb_rust: `--no-verify`/`--fix-padding` marked `[convert]`, `--keys` marked `[convert, split]` (merge needs no keys).
